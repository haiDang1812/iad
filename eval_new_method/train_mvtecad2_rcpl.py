# train_mvtecad2_dcpl.py
# Discriminative Contrast Prototype Learning (DCPL)
# Local Contrast + Orthogonal Prototype on MVTec AD 2
...
def build_model(args, device, embed_dim, num_heads,
    ...
    model = INP_Former(
        ...
        use_local_contrast=args.use_local_contrast,
...
def main(args):
    ...
    if args.phase == 'train':
        ...
        print_fn(f'use_local_contrast: {args.use_local_contrast}')
        print_fn(f'use_ortho_loss    : {args.use_ortho_loss}')
        print_fn(f'ortho_loss_weight : {args.ortho_loss_weight}')
        print_fn(f'neighborhood_sizes: {args.neighborhood_sizes}')
        ...
        for epoch in range(args.total_epochs):
            ...
            for img, _ in tqdm(train_dataloader, ncols=80):
                ...
                nn.utils.clip_grad_norm_(
                    trainable.parameters(), max_norm=0.1)
                optimizer.step()
                lr_scheduler.step()
        ...
        os.makedirs(
            os.path.join(args.save_dir, args.save_name, args.item),
            exist_ok=True)
        torch.save(
            model.state_dict(),
        ...
        ckpt = os.path.join(
            args.save_dir, args.save_name, args.item, 'model.pth')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='DCPL on MVTecAD-2')

    # Dataset
    parser.add_argument('--dataset',   type=str, default='MVTecAD-2')
    parser.add_argument('--data_path', type=str,
                        default=r'E:\dataset\mvtecad2')

    # Save
    parser.add_argument('--save_dir',  type=str, default='./saved_results')
    parser.add_argument('--save_name', type=str, default='DCPL-MVTecAD2')
    ...
    # Method
    parser.add_argument('--use_local_contrast', action='store_true',
                        default=True)
    parser.add_argument('--use_ortho_loss',     action='store_true',
                        default=True)
    parser.add_argument('--ortho_loss_weight',  type=float, default=0.1)
    parser.add_argument('--neighborhood_sizes', type=int, nargs='+',
                        default=[3, 5],
                        help='Neighborhood sizes for local contrast')
    ...
    args.save_name = (
        ...
        + f'_INP={args.INP_num}'
        + f'_LC={args.use_local_contrast}'
        + f'_Ortho={args.use_ortho_loss}'
        + f'_OrthoW={args.ortho_loss_weight}'
    )
