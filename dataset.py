...
class MVTecAD2Dataset(torch.utils.data.Dataset):
    ...
    def load_dataset(self):
        ...
        if self.phase == 'train':
            # train/good/
            train_good_path = os.path.join(self.root, 'train', 'good')
            img_paths = glob.glob(os.path.join(train_good_path, "*.png")) + \
                        glob.glob(os.path.join(train_good_path, "*.jpg")) + \
                        glob.glob(os.path.join(train_good_path, "*.JPG")) + \
                        glob.glob(os.path.join(train_good_path, "*.bmp"))
            img_tot_paths.extend(img_paths)
            gt_tot_paths.extend([0] * len(img_paths))
            tot_labels.extend([0] * len(img_paths))
            tot_types.extend(['good'] * len(img_paths))
            ...
            # test_public/good/
            test_good_path = os.path.join(self.root, 'test_public', 'good')
            if os.path.exists(test_good_path):
                img_paths = glob.glob(os.path.join(test_good_path, "*.png")) + \
                            glob.glob(os.path.join(test_good_path, "*.jpg")) + \
                            glob.glob(os.path.join(test_good_path, "*.JPG")) + \
                            glob.glob(os.path.join(test_good_path, "*.bmp"))
                img_tot_paths.extend(img_paths)
                gt_tot_paths.extend([0] * len(img_paths))
                tot_labels.extend([0] * len(img_paths))
                tot_types.extend(['good'] * len(img_paths))
            ...
            # test_public/bad/
            test_bad_path = os.path.join(self.root, 'test_public', 'bad')
            gt_bad_path = os.path.join(self.root, 'test_public', 'ground_truth', 'bad')
            if os.path.exists(test_bad_path):
                img_paths = glob.glob(os.path.join(test_bad_path, "*.png")) + \
                            glob.glob(os.path.join(test_bad_path, "*.jpg")) + \
                            glob.glob(os.path.join(test_bad_path, "*.JPG")) + \
                            glob.glob(os.path.join(test_bad_path, "*.bmp"))
                img_paths.sort()

                gt_paths = []
                for img_path in img_paths:
                    img_name = os.path.basename(img_path)
                    # MVTecAD-2 GT files have _mask suffix
                    name_without_ext = os.path.splitext(img_name)[0]
                    ext = os.path.splitext(img_name)[1]
                    gt_name = name_without_ext + '_mask' + ext
                    gt_path = os.path.join(gt_bad_path, gt_name)
                    gt_paths.append(gt_path)

                img_tot_paths.extend(img_paths)
                gt_tot_paths.extend(gt_paths)
                tot_labels.extend([1] * len(img_paths))
                tot_types.extend(['bad'] * len(img_paths))
