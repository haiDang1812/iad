...
import math
from models.vision_transformer import LocalContrastExtractor, OrthogonalPrototypeLoss
...
class INP_Former(nn.Module):
    def __init__(
        ...
        # ── NEW: Local Contrast Extractor ──
        if use_local_contrast:
            embed_dim = prototype_token[0].shape[-1]
            self.local_contrast = LocalContrastExtractor(
                dim=embed_dim,
                neighborhood_sizes=neighborhood_sizes,
                learnable_weight=True
            )

        # ── NEW: Orthogonal Prototype Loss ──
        if use_ortho_loss:
            self.ortho_loss_fn = OrthogonalPrototypeLoss()
    ...
    def forward(self, x):
        ...
        if self.remove_class_token:
            en_list = [e[:, 1 + self.encoder.num_register_tokens:, :]
                       for e in en_list]

        x = self.fuse_feature(en_list)

        # ── NEW: Local Contrast Enhancement ──
        if self.use_local_contrast:
            x = x + self.local_contrast(x, side)

        # ── INP Extractor ──
        agg_prototype = self.prototype_token
        for i, blk in enumerate(self.aggregation):
            agg_prototype = blk(
                agg_prototype.unsqueeze(0).repeat((B, 1, 1)), x)

        g_loss = self.gather_loss(x, agg_prototype)

        # ── NEW: Orthogonal Prototype Loss ──
        ortho_loss = torch.tensor(0., device=x.device)
        if self.use_ortho_loss:
            ortho_loss = self.ortho_loss_fn(agg_prototype)
        ...
        en = [e.permute(0, 2, 1).reshape(
            [x.shape[0], -1, side, side]).contiguous() for e in en]
        de = [d.permute(0, 2, 1).reshape(
            [x.shape[0], -1, side, side]).contiguous() for d in de]

        return en, de, g_loss, ortho_loss  # ← thêm ortho_loss
